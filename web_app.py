...
def build_config(form: Dict[str, str], attachments: Optional[List[str]] = None, require_password: bool = True) -> EngineConfig:
    ...
    hourly_limit = int(form.get("hourly_limit", "20").strip())
    ...
    if daily_limit <= 0 or hourly_limit <= 0:
        raise ValueError("Daily/hourly limits must be positive integers.")
...
def preview():
    try:
        ...
        engine = build_config(request.form, attachments=attachments, require_password=False)
...
def start():
    try:
        ...
        attachments = save_attachments_upload(batch_id)
        engine = build_config(request.form, attachments=attachments)
        csv_path = save_csv_upload()
...
def recipients():
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            q = """
                SELECT r.email, sl.status, sl.sent_at, sl.error
            ...
            data = [
                {"email": r[0], "status": r[1] or "pending", "sent_at": r[2], "error": r[3]}
                for r in rows
            ]
