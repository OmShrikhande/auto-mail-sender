...
class EngineConfig:
    ...
    # Human-like delays. If not provided, they are derived from hourly_limit.
...
def _count_sent_in_hour(conn: sqlite3.Connection, engine: EngineConfig, day: str, hour: int) -> int:
    # sent_at is local ISO time; we can filter by prefix for hour.
    prefix = f"{day}T{hour:02d}:"
    q = "SELECT COUNT(*) FROM send_log WHERE day_key = ? AND status = 'sent' AND sent_at LIKE ? || '%'"
    return int(conn.execute(q, (day, prefix)).fetchone()[0])


def _derive_delay_bounds(engine: EngineConfig) -> Tuple[int, int]:
    hourly = max(1, engine.hourly_limit)
    avg = int(3600 / hourly)
    min_delay = engine.min_delay_sec or max(20, int(avg * 0.6))
    max_delay = engine.max_delay_sec or max(min_delay + 1, int(avg * 1.6))
    # Apply jitter range to look more human.
    min_delay = max(5, int(min_delay * (1 - engine.jitter_factor)))
    max_delay = max(min_delay + 5, int(max_delay * (1 + engine.jitter_factor)))
    return min_delay, max_delay


def _smtp_send(engine: EngineConfig, to_email: str, subject: str, body: str) -> None:
    # Keep it simple: plain text only (better transparency, fewer spam heuristics than complex HTML).
    msg = EmailMessage()
    ...
    with sqlite3.connect(engine.db_path) as conn:
        ...
        while True:
            ...
            if day_sent >= engine.daily_limit:
                report("completed")
                break
            hour_sent = _count_sent_in_hour(conn, engine, day, hour)
            if hour_sent >= engine.hourly_limit:
                next_hour = (now.replace(minute=0, second=0, microsecond=0) + timedelta(hours=1))
                report("sleeping", next_at=next_hour)
                time.sleep(max(5, min(60, int((next_hour - now).total_seconds()))))
                continue
